...
function persistMemberChange(member) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        Member.findByIdAndUpdate(member._id, member, MONGOOSE_UPDATE_OPTIONS, function (err) {
        ...
        Member.find({
            ...
            if (err)
                ...
                if (members.length == 1) {
                    ...
                    (members.length > 1) ? reject('Signup is ambiguous') : reject('Signup Unsuccessful');
            }
        });
    });
}
            ...
            for (var i = 0; i < members.length; ++i) {
                {
                    ...
                    Member.findByIdAndUpdate(members[i]._id, members[i], MONGOOSE_UPDATE_OPTIONS, function (err, returned_member) {
                        if (err) {
